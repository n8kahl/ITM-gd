...
**Version:** 1.2.0
**Date:** 2026-02-13
**Status:** Repo-aligned with scope lock + WebSocket-first real-time rules + FRED macro integration
...
**Supersedes:** `ITM_Live_Macro_Data_Integration_Spec.docx` (archive after this spec is accepted)
...
### 0.8 Live Macro Data (FRED) validation — feasibility check

The prior Live Macro Data plan (Massive Economy + FRED + Benzinga earnings) is technically compatible with this repo, with two required guardrails:

1) **Macro service is currently synchronous and mostly curated/hardcoded.**
- `backend/src/services/macro/macroContext.ts` currently returns static/derived values synchronously.
- `backend/src/routes/macro.ts`, `backend/src/chatkit/functionHandlers.ts`, and `backend/src/services/morningBrief/index.ts` consume those sync functions.

2) **Integration must preserve response shape while moving data fetch async.**
- Keep existing output contracts (`MacroContext`, macro impact payloads) stable.
- Add async provider-backed methods and explicit fallback chain:
  - live provider result
  - Redis cache (or in-memory fallback when Redis unavailable)
  - current static macro context as last-resort fallback

**Validation outcome:** FRED integration is viable without frontend breakage if the migration is staged as “async internals + stable external shape.”
...
This document provides an autonomous implementation plan to upgrade the TITM AI Coach platform from ~6 Massive.com endpoint families to a **high-value, production-safe endpoint set first**, with optional add-on packs (including live macro/FRED) after baseline stability.
...
| 3 | AI Coach Enrichment (+ Live Macro/FRED) | 8 tasks | MEDIUM |
...
massive.ts (EXPANDED — ~1400 lines)
  ...
  ├── [NEW] Macro Economy Provider (Massive economy + treasury yields)
  ├── [NEW] Macro Series Provider (FRED: GDP, CPI/PPI proxies, labor, rates)
  ...
  [NEW] Macro Data Layer (provider normalization + cache + fallback)
       ↓
...
| 24 | Economic Series (`/fred/series/observations`, external) | HIGH | 3 |
...
| 11 | Macro context uses static/derived values, not live data | HIGH | macroContext.ts + macro consumers |
...
> **Goal:** Make the AI Coach dramatically smarter by giving it access to fundamentals, news context, richer symbol data, and live macroeconomic context.
...
### Task 3.8 — Live Macro Data Integration (Massive Economy + FRED)

**Objective:** Replace curated macro placeholders with live provider data while preserving existing macro response contracts consumed by routes, ChatKit handlers, and morning brief generation.

**Create (backend):**
- `backend/src/services/macro/providers/massiveEconomy.ts`
- `backend/src/services/macro/providers/fredClient.ts`
- `backend/src/services/macro/macroCache.ts`
- `backend/src/services/macro/__tests__/fredClient.test.ts`

**Modify (backend):**
- `backend/src/services/macro/macroContext.ts` (becomes orchestrator with async live path + fallback)
- `backend/src/routes/macro.ts` (await live macro functions)
- `backend/src/chatkit/functionHandlers.ts` (`get_macro_context` handler awaits live macro data)
- `backend/src/services/morningBrief/index.ts` (await live macro context)
- `backend/src/config/env.ts` (add FRED + macro cache vars)

**Required contract strategy (repo-safe):**
- Keep shape compatibility for existing `MacroContext` output.
- Introduce async methods:
  - `getMacroContextLive(): Promise<MacroContext>`
  - `assessMacroImpactLive(symbol: string): Promise<MacroImpact>`
- Keep sync static fallback helper for emergency degradation:
  - `getMacroContextStaticFallback()`

**Provider scope for first cut:**
- Massive economy/treasury endpoints for market-side macro fields where available.
- FRED series for gap-fill and hard requirements (GDP, unemployment, rates; extend as needed).

**Fallback chain (must implement):**
1. Fresh provider fetch  
2. Redis cached payload  
3. Static fallback payload (current behavior) with freshness metadata

**Test:**
- Unit test: FRED series mapping and numeric parsing.
- Unit test: fallback chain (provider fail → cache → static fallback).
- Integration test: `GET /api/macro` returns non-null live-populated fields when provider responses are available.
- Integration test: no `500` on provider failure; returns fallback + freshness flag.
...
- Existing related env vars already in use by the repo and affected by this work:
  ...
  - macro service currently has no provider key for FRED and no macro cache toggles
...
FRED_API_KEY=your_fred_api_key        # Free key from St. Louis Fed
FRED_BASE_URL=https://api.stlouisfed.org/fred
MACRO_CACHE_ENABLED=true              # Cache macro provider payloads
MACRO_CONTEXT_TTL_SECONDS=3600        # Macro context TTL
...
MACRO_CACHE_ENABLED: z.coerce.boolean().optional().default(true),
MACRO_CONTEXT_TTL_SECONDS: z.coerce.number().optional().default(3600),
```
...
- [ ] `/api/macro` returns live-populated macro fields from Massive/FRED when providers are available
- [ ] Macro provider failures degrade to cached/static fallback without returning `500`
...
backend/src/services/macro/providers/massiveEconomy.ts   (Phase 3.8)
backend/src/services/macro/providers/fredClient.ts       (Phase 3.8)
backend/src/services/macro/macroCache.ts                 (Phase 3.8)
backend/src/services/macro/__tests__/fredClient.test.ts  (Phase 3.8)
...
backend/src/services/macro/macroContext.ts       (Phase 3.8 — live provider orchestration)
backend/src/routes/macro.ts                      (Phase 3.8 — async live macro route path)
backend/src/services/morningBrief/index.ts       (Phase 3.8 — await live macro data)
...
Phase 3 (AI Coach)
  ...
  └── Task 3.8: Live Macro (FRED + Economy) (depends on 3.6)
...
**Phase 3:** Tasks 3.1, 3.2 can run in parallel; 3.3-3.7 depend on earlier work; 3.8 should run after 3.6 to preserve contract stability
