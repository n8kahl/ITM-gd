...
### Commands
- `npm test -- --runInBand src/routes/__tests__/brief.test.ts src/routes/__tests__/scanner.test.ts src/routes/__tests__/watchlist.test.ts src/routes/__tests__/trackedSetups.test.ts src/workers/__tests__/setupPushWorker.test.ts`
...
- Setup push worker scaffolding is running with startup/shutdown hooks.

### Needs Completion
- Scheduled morning brief job (cron/worker path) with idempotency.
- Setup detector engine + real-time setup push channels connected to worker/channel scaffolding.
- Full E2E path:
  - scanner -> track setup -> manage tracked setup -> morning brief consume.

## 4) Surgical Next Plan

1. Connect setup detector output to setup push worker/channel and deliver user-targeted events.
2. Add Playwright E2E smoke for scanner -> track -> tracked-setups lifecycle -> brief consume.
