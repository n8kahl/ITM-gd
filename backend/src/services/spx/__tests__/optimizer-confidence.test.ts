...
describe('optimizer confidence controls', () => {
  ...
  it('does not adjust minEvR when rolling slippage is below threshold', () => {
    const adjustment = __optimizerTestUtils.computeSlippageGuardrailAdjustment({
      sampleCount: 5,
      minSamples: 5,
      avgEntrySlippagePts: 0.18,
      thresholdPts: 0.25,
      currentMinEvR: 0.3,
      bumpEvR: 0.05,
      maxMinEvR: 0.8,
      windowSignature: '1|2|3|4|5',
      lastAppliedWindowSignature: null,
    } as any);

    expect(adjustment.shouldAdjust).toBe(false);
    expect(adjustment.nextMinEvR).toBe(0.3);
  });

  it('adjusts minEvR when rolling slippage exceeds threshold', () => {
    const adjustment = __optimizerTestUtils.computeSlippageGuardrailAdjustment({
      sampleCount: 5,
      minSamples: 5,
      avgEntrySlippagePts: 0.31,
      thresholdPts: 0.25,
      currentMinEvR: 0.3,
      bumpEvR: 0.05,
      maxMinEvR: 0.8,
    ...
    expect(adjustment.shouldAdjust).toBe(true);
    expect(adjustment.nextMinEvR).toBe(0.35);
  });

  it('does not re-apply when the same slippage window signature was already applied', () => {
    const adjustment = __optimizerTestUtils.computeSlippageGuardrailAdjustment({
      sampleCount: 5,
      minSamples: 5,
      avgEntrySlippagePts: 0.31,
      ...
      bumpEvR: 0.05,
      maxMinEvR: 0.8,
      windowSignature: 'same-window',
      lastAppliedWindowSignature: 'same-window',
    } as any);
