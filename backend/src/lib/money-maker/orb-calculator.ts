 ...
 */
export function computeORB(bars: CandleBar[]): ORBData | null {
 ...
 */
export function determineRegime(currentPrice: number, orb: ORBData | null): ORBRegime {
    if (!orb) return 'choppy' // default to choppy if no ORB is established yet
