...
export async function GET(
  ...
  try {
    ...
    const upstream = `${backendBase}/api/levels/${encodeURIComponent(symbol)}${url.search}`
    ...
    if (!response.ok) {
      return new NextResponse(
        payload || JSON.stringify({ message: 'Failed to fetch key levels.' }),
        {
    ...
    return NextResponse.json(
      { error: 'proxy_error', message: 'Failed to fetch key levels.' },
      { status: 502 },
    )
  }
}
