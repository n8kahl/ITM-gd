...
export interface PatienceCandleResult {
    ...
    reason?: string // for debugging when false
}
 ...
 * $2,000+ (SPX): ±$2.00
 */
export function getDynamicBandTolerance(price: number): number {
    if (price < 20) return 0.05 // Failsafe for very cheap stocks
    if (price <= 80) return 0.10
    if (price <= 100) return 0.20
    if (price <= 300) return 0.30
