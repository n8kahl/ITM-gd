...
export const aiPreferencesSchema = z.object({
  ...
  preferred_symbols: z.array(
    z.string().trim().toUpperCase().regex(/^[A-Z0-9./]{1,16}$/)
  ).max(20),
...
export const memberProfileUpdateSchema = z.object({
  display_name: z.string().trim().max(50).nullable().optional(),
...
export const createFeedItemSchema = z.object({
  ...
  reference_table: z.string().min(1).max(50),
