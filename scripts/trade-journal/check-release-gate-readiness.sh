...
WORKFLOW_PATH=".github/workflows/trade-journal-release-gates.yml"
...
print_header() {
  echo "Trade Journal Release Gate Preflight"
  echo "Repo: ${REPO}"
...
check_workflow_exists() {
  if [[ "${gh_ready}" != "1" ]]; then
    echo "WARN: skipping workflow checks (gh not authenticated)."
    warnings=$((warnings + 1))
    return
  fi
  ...
  if [[ "${TARGET_BRANCH}" != "main" ]]; then
    if gh api "repos/${REPO}/contents/${WORKFLOW_PATH}?ref=main" >/dev/null 2>&1; then
      ...
      echo "WARN: ${WORKFLOW_PATH} not found on main."
      warnings=$((warnings + 1))
    fi
  fi
}
...
check_secrets() {
  ...
  if [[ -z "${secret_list}" ]]; then
    echo "WARN: repository secrets are not visible via gh for ${REPO}."
...
print_summary() {
  ...
  if [[ "${failures}" == "0" ]]; then
    echo "READY: Trade Journal release gate can be dispatched."
  else
    echo "NOT READY: resolve failures before dispatching the release gate."
    exit 1
  fi
}
