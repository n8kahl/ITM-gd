...
function getTierColor(tier: string | null | undefined): string {
  ...
  if (t.includes('elite') || t.includes('diamond')) return 'text-cyan-300 border-cyan-500/30'
  if (t.includes('gold') || t.includes('premium')) return 'text-yellow-400 border-yellow-500/30'
  if (t.includes('silver') || t.includes('pro')) return 'text-gray-300 border-gray-400/30'
...
export function FeedItemCard({ item, className }: FeedItemCardProps) {
  ...
  const avatarUrl = author?.discord_avatar || null
